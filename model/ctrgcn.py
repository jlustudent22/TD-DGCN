...
class CTRGC(nn.Module):
    ...
    def forward(self, x, A=None, alpha=1):
        global iii
        torch.save(x.mean(0).mean(0),'/home/wangsq/tmm/zhengli1/2/x{}.pth'.format(str(iii)))
        x1, x2 = self.conv1(x), self.conv2(x)
        x3= self.conv(x, self.conv_rf(x))
        torch.save(x3.mean(0).mean(0),'/home/wangsq/tmm/zhengli1/2/xx{}.pth'.format(str(iii)))
        x3 = torch.einsum('nctv,vu->nctu', x3, A)
        #print('x3',x3.shape)
        xx = x.mean(0).mean(0)
        #print('xx',xx.shape,x1.shape)
        xxx = torch.einsum('nctv,tu->ncvu', x1, xx)
        #xxx = torch.matmul(x1,(xx.T))
        #print('xxx',xxx.shape)
        xx1 = xxx.mean(0).mean(0)
        xxx = x2*torch.diagonal(xx1)
        #print(xxx.shape)
        x1 = x3 + alpha*xxx
        torch.save(x1.mean(0).mean(0),'/home/wangsq/tmm/zhengli1/2/xxx{}.pth'.format(str(iii)))
        iii = iii + 1
